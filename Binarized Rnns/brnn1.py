...
class RNNClassifier(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, output_size, n_layers=1, bidirectional=True):
        ...
        self.gru = Rn.BGRU(hidden_size, hidden_size, n_layers,
                          bidirectional=bidirectional)
        self.fc = BinaryLinear(hidden_size, output_size)
...
def train():
    ...
    for i, (names, countries) in enumerate(train_loader, 1):
        ...
        for name, all_weights in classifier.named_parameters():
            #print(name)
            if hasattr(all_weights,'org'):
                all_weights.data.copy_(all_weights.org)
                #all_weights.data.copy_=all_weights.org.data
        ...
        for name, all_weights in classifier.named_parameters():
            #print(name)
            if hasattr(all_weights,'org'):
                #print("???????????????????????????????",all_weights.org)
                all_weights.org.copy_(all_weights.data.clamp_(-1,1))
    
        if i % 10 == 0:
            tlos.append(total_loss)
...
def test(name=None):
    ...
    accur.append( 100.*correct/train_data_size)
...
if __name__ == '__main__':
    ...
    #print(time_graph)
    plt.title('Training for name dataset with epoch', fontsize=20)
    plt.ylabel('time (s)')
